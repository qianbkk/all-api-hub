...
const changelogSourceUrls = vi.hoisted(() => ({
  ...
  rawIndex: "https://raw.example.test/main/changelog-index.json",
...
`)
  ...
  it("falls back to the main-branch raw index when the docs index is unavailable", async () => {
  ...
  it("falls back to the main-branch raw index when docs index body parsing stalls", async () => {
    vi.useFakeTimers()
