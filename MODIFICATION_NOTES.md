...
- Chromium 生产构建已完成 Vite 打包阶段，仅有既有的动态/静态混合导入和大 chunk 警告；本机 WXT 进程在最终收尾阶段长时间未退出，因而终止等待，未将其误记为完整通过。
- Knip 在当前本机环境无法解析项目 `tsconfig.json`（同一配置可被 TypeScript 正常解析），未将环境性失败视为代码通过。

文档发布地址为 `https://qianbkk.github.io/all-api-hub/`，发布工作流仍部署到本仓库的 `gh-pages` 分支。CI 会在干净依赖环境中重新执行完整测试和构建，以上未完成项应以远端 CI 结果为最终依据。
