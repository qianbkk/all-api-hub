...
**All API Hub** ([open source on GitHub](https://github.com/qianbkk/all-api-hub)) is a browser extension for AI API users who need to manage multiple accounts, sites, and client configurations. For Xingchen AI users, it brings account status, API keys, model pricing, and export actions into one workflow.
