...
**All API Hub**（[GitHub 开源](https://github.com/qianbkk/all-api-hub)）是一款面向 AI API 用户的开源浏览器扩展，适合用来集中管理多个账号、多个站点和多个客户端配置。对 RunAPI 用户来说，它可以把 RunAPI 的账号信息、API 密钥、模型价格和导出配置纳入统一工作流。
